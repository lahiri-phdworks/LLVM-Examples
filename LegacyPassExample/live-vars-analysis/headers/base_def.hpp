 ...
 * https://llvm.org/doxygen/LiveVariables_8cpp_source.html This file contains
 * all the ADTs and struct definitions needed for Live Variable Analysis.
 */
...
template <typename T> std::set<T> union_set(std::set<T> s1, std::set<T> s2) {
  std::set<T> result;
...
template <typename T> std::set<T> diff_set(std::set<T> s1, std::set<T> s2) {
  std::set<T> result;
  result.insert(s1.begin(), s1.end());
...
class Dataflow {
public:
  Dataflow(std::vector<void *> domain) : domain(domain){};
  std::set<int> MeetOperationSets(std::vector<std::set<int>> input);
  std::map<BasicBlock *, BlockResult>
  pre_analysis(Function &F, std::set<int> boudary, std::set<int> interior);
  int domainIndex(void *ptr);
  virtual BlockTransferFunction transferFn(std::set<int> input,
