 ...
 * https://llvm.org/doxygen/LiveVariables_8cpp_source.html LVA Analysis on MAIN
 * Function.
 */
...
std::vector<std::set<std::string>>
    liveSetElems; // Store the live vars sets at each program point.
std::vector<std::pair<Instruction *, std::set<std::string>>>
    output;                             // Stores the processed output.
...
public:
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    std::vector<void *> domain; // global domain, all possible values. init to
                                // TOP like in lattice.

    // add all used values to the domain set, initialize to TOP (in LVA, it is
    // set of all variables --> domain)
    for (auto I = inst_begin(F); I != inst_end(F); ++I) {
      if (Instruction *inst = dyn_cast<Instruction>(&*I)) {
        for (auto operands = inst->op_begin(); operands != inst->op_end();
             ++operands) {
          Value *operand = *operands;

          // Check operand type isa<X> and type_name as well.
          if (isa<Instruction>(operand) || isa<Argument>(operand)) {
            if (std::find(domain.begin(), domain.end(), operand) ==
                domain.end()) {
              domain.push_back(
                  operand); // if operand, we push back to domain set.
            }
          }
        }
      }
    ...
    // We have got in/out for each block, now we need to analyze each
    // instruction
    for (auto &BB : F) {

      if (F.getName() == "main") { // analysing only for the main function.

        errs() << "Basic Block : " << BB.getName() << "\n";

        std::set<int> live =
            fresult[&BB].out; // out contains the Live Variables list.

        for (auto inst = BB.rbegin(); inst != BB.rend();
             ++inst) { // backward analysis.
          Instruction *I = &*inst;
            ...
            for (auto op = inst->op_begin(); op != inst->op_end(); ++op) {
              ...
              if (isa<Instruction>(val) || isa<Argument>(val)) {
                ...
                if (i != analysis.INDEX_NOT_FOUND) {
                  live.insert(i); // insert, (duplicates are automatically
                                  // rejected at insert(). )
                }
              }
            }
        ...
        for (auto o = output.rbegin(); o != output.rend(); ++o) {
          ...
          errs() << "Program Point " << counter++ << ", instr :" << *o->first
                 << ", live_set = ";
...
char Liveness::ID = 0;
static RegisterPass<Liveness> X("liveness", "Live Variable Analysis Pass", true,
                                true);
